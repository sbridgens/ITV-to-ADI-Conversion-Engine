...
/*Thread thread;
        ADIWF_Controller _Controller;
        private static readonly log4net.ILog log = log4net.LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);

        public ADIWFE_Service()
        {
            InitializeComponent();
            this.ServiceName = "ADI_WFE";
            this.CanHandleSessionChangeEvent = true;
            this.CanPauseAndContinue = false;
            this.CanHandlePowerEvent = true;
            this.CanShutdown = true;
        ...
        protected override void OnStart(string[] args)
        {
            ...
            {
                System.IO.Directory.SetCurrentDirectory(System.AppDomain.CurrentDomain.BaseDirectory);
                _Controller = new ADIWF_Controller();
                thread = new Thread(_Controller.WF_Start);
                thread.Start(_Controller);
                base.OnStart(args);
        ...
        private void T_Start() => thread.Start(_Controller);

        private void Base_Start(string[] args) => base.OnStart(args);
*/